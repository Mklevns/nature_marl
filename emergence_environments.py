...
class EmergenceEnvironment(ParallelEnv, ABC):
    ...
    def __init__(self, n_agents: int = 5, grid_size: Tuple[int, int] = (20, 20),
    ...
    def observation_space(self, agent: str) -> spaces.Space:
        ...
        return spaces.Box(low=-1, high=1, shape=(base_obs_dim + comm_obs_dim,), dtype=np.float32)
...
class ForagingEnvironment(EmergenceEnvironment):
    ...
    def __init__(self, n_agents: int = 5, grid_size: Tuple[int, int] = (30, 30),
        ...
        super().__init__(n_agents, grid_size, **kwargs)

    def setup_communication_channel(self):
        """Use pheromone field for ant-like communication."""
        self.communication_channel = PheromoneField(
            self.grid_size,
            n_chemicals=2,  # One for food trail, one for exploration
            decay_rate=0.98,
            diffusion_rate=0.05
        )
...
class PredatorPreyEnvironment(EmergenceEnvironment):
    ...
    def setup_communication_channel(self):
        ...
        self.communication_channel = SymbolicChannel(
            vocab_size=8,  # Different alarm types
    ...
    def step(self, actions: Dict[str, np.ndarray]):
        ...
        for agent, action in actions.items():
            if agent in self.agents and len(action) > 5:
...
class TemporalCoordinationEnvironment(EmergenceEnvironment):
    ...
    def setup_communication_channel(self):
        ...
        self.communication_channel = PheromoneField(
            self.grid_size,
            n_chemicals=1,  # Single timing signal
...
class InformationAsymmetryEnvironment(EmergenceEnvironment):
    ...
    def __init__(self, n_agents: int = 4, n_info_types: int = 3, **kwargs):
    ...
    def setup_communication_channel(self):
        ...
        self.communication_channel = SymbolicChannel(
            vocab_size=self.n_info_types * 4,  # Type + value encoding
    ...
    def _get_obs(self, agent: str) -> np.ndarray:
        ...
        base_obs_size = 10  # As defined in observation_space
        knowledge_size = min(len(knowledge_vector), base_obs_size - len(basic_obs_components))
        basic_obs_components.extend(knowledge_vector[:knowledge_size])

        # Pad if needed
        while len(basic_obs_components) < base_obs_size:
            basic_obs_components.append(0.0)

        basic_obs = basic_obs_components[:base_obs_size]  # Ensure exact size
