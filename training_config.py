...
class TrainingConfigFactory:
    ...
    def create_gpu_config(self, obs_space, act_space) -> PPOConfig:
        ...
        config = (
            ...
            .env_runners(
                num_env_runners=4,  # Conservative worker count
                num_envs_per_env_runner=1,  # Single env per worker for stability
                num_cpus_per_env_runner=1,
                sample_timeout_s=120,  # Allow time for communication processing
            )
            .learners(
                num_learners=1,  # Single GPU learner
                num_gpus_per_learner=1,
                num_cpus_per_learner=2,  # CPU support for GPU
            )
            .rl_module(
            ...
            .training(
                ...
                train_batch_size_per_learner=1024,  # Conservative for memory
                minibatch_size=128,  # Smaller minibatches for GPU memory
                lr=3e-4,
                gamma=0.95,
                lambda_=0.95,
                clip_param=0.2,
                vf_loss_coeff=0.5,
                entropy_coeff=0.01,
                num_epochs=8,  # Reduced epochs to prevent memory buildup
                grad_clip=0.5,
                use_critic=True,
                use_gae=True,
            )
    ...
    def create_cpu_config(self, obs_space, act_space) -> PPOConfig:
        ...
        config = (
            ...
            .env_runners(
                num_env_runners=max_workers,
                num_envs_per_env_runner=1,
                num_cpus_per_env_runner=1,
                sample_timeout_s=120,
            )
            .learners(
                num_learners=2,  # Multiple CPU learners for parallel training
